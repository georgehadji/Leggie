...
| Metric | subset7 | subset8 |
|---|---|---|
| parse-failure rate | 11/76 = 14.5% | 8/89 = 9.0% |
| findings | 7 (0.70/art.) | 7 (0.70/art.) |
| truncations | 0 | 0 |

> **RETRACTED 2026-07-18 — this table is NOT evidence for D18.** Per §D20, both
> runs executed **main-checkout** code, which does not contain D18. They are two
> runs of *identical* code, so the 14.5% → 9.0% delta measures **run-to-run
> noise**, not the fix. Treat `[9.0%, 14.5%]` as the observed noise band for this
> metric at n=10 articles — which is wide enough that **no single-run comparison
> can establish a parse-failure improvement.** Any future D18 verdict needs
> repeated runs on both arms, not one each.

D18's *mechanism* remains independently proven offline (the parser demonstrably
rejected out-of-range payloads before the change and clamps them after, with 4
regression tests). What is unproven is its **aggregate effect on the live
parse-failure rate** — that measurement has not yet been taken with the fix
actually loaded.

### D20 — every live run in this session executed MAIN-CHECKOUT code, not the worktree — **all session live numbers invalid**

The `leggie` console script resolves imports through the editable-install
finder, whose mapping is:

```
MAPPING: {'leggie': 'E:\\Documents\\Vibe-Coding\\Leggie\\leggie'}   # main checkout
```

A console script does **not** put cwd on `sys.path`, so running `leggie analyze`
from the worktree still imports the **main checkout**. Verified directly:

| invocation | resolves to | D19 present |
|---|---|---|
| `leggie ...` (what every run used) | `...\Leggie\leggie\...` (main) | **False** |
| `PYTHONPATH=<worktree> leggie ...` | `...\worktrees\bold-nightingale-be6980\leggie\...` | True |

**Consequence:** subset8 and all four Phase-D per-lens runs did not exercise
D12/D18/D19. This single cause explains three otherwise-puzzling observations
at once — the hallmark of a correct root cause:
1. **Zero `skeptic_route_*` log lines** despite D19 adding them at INFO and
   passing unit tests — the code simply wasn't loaded.
2. **Truncations at exactly 1024 and 2048 tokens** — main's hardcoded
   `_DEFAULT_*_MAX_TOKENS` fallbacks. D12 (honour `route.max_tokens`) is
   worktree-only, so `routes.yaml`'s 8192 could never apply.
3. **The D18 "improvement"** — two runs of the same code (see retraction above).

It also retroactively casts doubt on **every** live measurement in this campaign
that was taken via the bare `leggie` CLI from this worktree, including the
subset3/5/6/7 comparisons. Those runs measured main-checkout behaviour; where
they moved, the cause was config (`config/routes.yaml`, loaded from cwd) or
noise — **not** the worktree code changes.

**Correct invocation going forward:** `PYTHONPATH=<worktree> leggie analyze ...`
(or `python -m` from the worktree). Every future live gate number in this
document must record which invocation produced it.
...
### Phase G status (2026-07-18)

| Step | Status |
|---|---|
| 1. Final offline sweep | **DONE** — 554 passed, ruff/mypy clean, `layer-dependencies KEPT` |
| 2. `docs/SMOKE_AUDIT_V3.md` | **DONE** — `13669eb`; logs committed to `docs/evidence/v3/` |
| 3. Close H-2 | **DONE** — `646d41b`; heuristic at `llm/__init__.py:239-244` + `test_repair_round_skipped_on_prose_garbage` (was already fixed; audit corrected) |
| 4. Commit in project style | **DONE** |
| 5. README drift | **DONE** — `646d41b`; badge 199→554, lines→9,880, ports→11, counts→554/47 |
| 6. Campaign skill state table | **DONE** — `35ecd81` (table) + `361701c` (frontmatter) |

**Phase E is PARTIAL, not closed.** `full5_v3` passes every §6 gate row, but on
articles 1–10, not the full 91-article bill the gate was written against. The
full run forecasts ~$5.01 against the fenced $5.00 cap with $8.17 credit
remaining — it would abort mid-flight. Blocked on a credit top-up, recorded as
OPEN in the audit rather than counted as a pass.

**Every paid run before `full5_v3` executed `main`, not this worktree** (D20).
Those numbers are a baseline of `main`. Any future measurement must set
`PYTHONPATH` to the worktree and prove it with `*_route_resolved` lines in the
log — see the audit doc §0.
