    ...
    def _analyze_regex(self, article: Article) -> list[Finding]:
        ...
        for pattern in _DELEGATION_PATTERNS:
            ...
            if m:
                findings.append(Finding(
                    finding_type=FindingType.CONSTITUTIONAL,
                    irac=IRAC(issue=f"Άρθρο {article.id}: Πιθανή υπέρβαση ορίων νομοθετικής εξουσιοδότησης",
                    ...
                    severity=Severity.HIGH, confidence=Confidence.from_score(0.6, provenance="regex-fallback"),
        ...
        for pattern in _RETROACTIVE_PATTERNS:
            ...
            if m:
                findings.append(Finding(
                    finding_type=FindingType.CONSTITUTIONAL,
                    irac=IRAC(issue=f"Άρθρο {article.id}: Πιθανή αναδρομική ισχύς",
                    ...
                    severity=Severity.MEDIUM, confidence=Confidence.from_score(0.5, provenance="regex-fallback"),
                    lens=self.name(), model="regex-fallback",
                    evidence=[Evidence(text_excerpt=m.group(0), verdict="supports")]))
        for pattern in _RIGHTS_PATTERNS:
            ...
            if m:
                findings.append(Finding(
                    finding_type=FindingType.CONSTITUTIONAL,
                    irac=IRAC(issue=f"Άρθρο {article.id}: Πιθανή επιρροή σε θεμελιώδη δικαιώματα",
                    ...
                    severity=Severity.HIGH, confidence=Confidence.from_score(0.55, provenance="regex-fallback"),
                    lens=self.name(), model="regex-fallback",
                    evidence=[Evidence(text_excerpt=m.group(0), verdict="supports")]))
        return findings
