...
_DELEGATION_NO_BODY_PATTERNS = [
    ...
    re.compile(
        r"(?:με\s+απόφαση\s+του\s+Υπουργού|με\s+πράξη\s+του|απόφαση\s+του\s+Διοικητή)",
        re.UNICODE | re.IGNORECASE,
    ),
]
...
_MISSING_MEASURES_PATTERNS = [
    re.compile(
        r"(?:εξουσιοδοτείται\s+να\s+εκδώσει|εκδίδεται\s+κανονιστική|προεδρικό\s+διάταγμα)",
        re.UNICODE | re.IGNORECASE,
    ),
    re.compile(
        r"(?:καθορίζεται\s+με|ρυθμίζεται\s+με|ορίζεται\s+με)\s+(?:απόφαση|πράξη|διάταγμα)",
        re.UNICODE | re.IGNORECASE,
    ),
]
                ...
                findings.append(
                    Finding(
                        ...
                        irac=IRAC(
                            ...
                            rule=(
                                "Οι προθεσμίες εφαρμογής πρέπει να είναι εύλογες και ρεαλιστικές"
                            ),
                            application=(
                                f"Το Άρθρο {article.id} ορίζει προθεσμία/έναρξη ισχύος "
                                "που μπορεί να μην είναι επαρκής"
                            ),
                            conclusion=(
                                f"Το Άρθρο {article.id} χρήζει ανάλυσης επάρκειας προθεσμιών"
                            ),
                        ),
                ...
                findings.append(
                    Finding(
                        ...
                        irac=IRAC(
                            ...
                            conclusion=(
                                f"Οι μεταβατικές διατάξεις του Άρθρου {article.id} "
                                "χρήζουν εξέτασης"
                            ),
                        ),
