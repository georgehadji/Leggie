...
class StaticRouter(RouterPort):
    ...
    async def route(self, task_type: str, _budget_remaining: float | None = None) -> RouteResult:
        """Route a task type to the configured model/tier."""
        rule = self._rules.get("routes", {}).get(task_type, {})
        if not rule:
            # Default fallback
            return RouteResult(
                model="claude-sonnet-4-20250514", tier=ModelTier.BUDGET, max_tokens=4096
            )
    ...
    async def cascade(
        self, task_type: str, current_tier: ModelTier, _failure_reason: str | None = None
    ) -> RouteResult | None:
