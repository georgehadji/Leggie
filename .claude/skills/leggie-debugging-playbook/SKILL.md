...
| 2 | Nonsense article numbers (552, 622Γ) or titles cut mid-word | parser matching in-body cross-references to other laws (historical, FIX_PLAN D2) | `leggie parse <file> -o parsed.json` → inspect article ids vs the actual bill TOC | `leggie/infrastructure/parse/__init__.py` `ARTICLE_HEADING` regex; settled fix in commit 703651e — check for regression |
...
| 12 | Crash mid-run, restart re-bills completed stages | should NOT happen when `--checkpoint PATH` was used: full-state resume has been implemented since `02c3ac6` (2026-07-11). This row previously said otherwise and was wrong | `python -m pytest tests/unit/application/test_bill_analysis_flow.py::TestResumeAfterCrash -q` — green = resume works; also check the checkpoint JSON has a `stage` key (full format) rather than only `tokens_used` (legacy budget-only format) | re-run with the SAME `--checkpoint PATH` and the same input file; resume is skipped if the file path differs, or if the saved stage is not in `FlowStateMachine.resumable_states()` (FAILED and terminal states never resume) |
