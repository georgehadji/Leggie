...
| DH-42 | verification chain eats 93% of findings: 27 reach the skeptic → 19 refuted (70%, vs 47% in v5) → 6 of the remaining 8 dropped by CoVe as `cove_quote_fail` → 2 survive. 0.02 findings/article vs v5's 0.14 | **OPEN, HIGH** — opened 2026-09-08 by the single-lens smoke; evidence in `docs/POST_DH36_REVIEW_PLAN.md` §12. Not DH-37 fallout (zero citation-gate drops that run). Suspect the lens fabricating `verbatim_quote`s (128/200 calls on gemini-2.5-flash) and/or an over-eager `adversarial_critic`. **Do not fix by loosening a gate** — that recreates the 68%-filler pathology (§3). |
