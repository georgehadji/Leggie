...
| D10 | Resume-from-stage: **CLOSED (this row was itself the bug).** `_save_checkpoint` persists the FULL run state — findings, events, document, source_text, suggestions, reports, budget_state, keyed by stage — on every `_transition()`, and `_load_checkpoint` re-enters at the saved stage. In place since `02c3ac6` (2026-07-11), i.e. before this skill's own 2026-07-14 snapshot. "Only budget spend" describes `_load_legacy_budget_checkpoint`, the backward-compat branch for pre-`02c3ac6` files. Corrected 2026-09-09; already recorded in `docs/DEFECT_HUNT_PLAN.md:169` on 2026-09-05 | CLOSED |
